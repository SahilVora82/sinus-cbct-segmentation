...
OUTPUT_DIRECTORY = Path(
    "data/test_predictions_fixed_normalization_threshold_091"
)

CSV_PATH = Path(
    "test_metrics_fixed_normalization_threshold_091.csv"
)
...
def load_and_normalize_scan(scan_path: Path):
    ...
    # Use the same fixed training-set statistics that were
...
def main():
    ...
    if not TEST_SCAN_DIRECTORY.exists():
        raise FileNotFoundError(
            f"Test scan directory was not found: "
            f"{TEST_SCAN_DIRECTORY}"
        )

    if not TEST_MASK_DIRECTORY.exists():
        raise FileNotFoundError(
            f"Test mask directory was not found: "
            f"{TEST_MASK_DIRECTORY}"
        )
    ...
    if len(scan_paths) == 0:
        raise RuntimeError(
            f"No NRRD test scans were found in "
            f"{TEST_SCAN_DIRECTORY}"
        )
    ...
    with torch.no_grad():
        for scan_path in scan_paths:
            ...
            if not mask_path.exists():
                raise FileNotFoundError(
                    f"Matching test mask was not found for "
                    f"{scan_path.name}: {mask_path}"
                )
            ...
            mask_array, mask_header = nrrd.read(
                str(mask_path)
            )
    ...
    average_dice = (
        sum(row["dice"] for row in results)
        / len(results)
    )

    average_iou = (
        sum(row["iou"] for row in results)
        / len(results)
    )

    average_precision = (
        sum(row["precision"] for row in results)
        / len(results)
    )

    average_recall = (
        sum(row["recall"] for row in results)
        / len(results)
    )
    ...
    print("=" * 45)
    print("FIXED-NORMALIZATION TEST RESULTS")
